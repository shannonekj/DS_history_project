...
.libPaths("/home/sejosiln/Programs/R/x86_64-pc-linux-gnu-library/3.0/")
library(optparse)
library(ggplot2)
...
option_list <- list(make_option(c('-i','--in_file'), action='store', type='character', default=NULL, help='Input file (output from ngsCovar)'),
                    ...
                    make_option(c('-s','--spp'), action='store', type='character', default=NULL, help='Species (will be DS)'),
...
PC$YEAR <- factor(annot$CLUSTER) #changed from Dan's "LOC"
PC$BATCH <- factor(annot$IDVAR) #changed from Dan's RT
...
#	legend.title=element_text(size=6.75)) +
  geom_point(data=PC,
	size =2,
	aes_string(x=paste("PC",comp[1],sep=""),
	y=paste("PC",comp[2],sep=""), color="YEAR")) +
  xlab(label = paste("PC",comp[1])) +
  ylab(label = paste("PC",comp[2]))
  #scale_color_manual("Delta Smelt Collection (Year)",breaks = c("Sil", "Ump","New","Eel","Sco"), values = c('Sco' = '#4daf4a','Eel' = '#ff7f00','New' = '#e41a1c', 'Sil' = '#984ea3', 'Ump' = '#377eb8'), labels = c("ORC (Siletz River)", "ORC (North Umpqua River)", "KMP (New River)","NCC (Eel River)", "CCC (Scott Creek)")) +
  #scale_shape_manual("Batch Number", breaks = c("P", "M"), values = c('P'= 17, 'M' = 16), labels = c("Premature", "Mature"))
} else if (spp == "Ch") {
