...
#SBATCH -J doPCA
#SBATCH -e 18_doPCA.allYears.%j.err
#SBATCH -o 18_doPCA.allYears.%j.out
#SBATCH -c 20
...
### Make bamlist
ls ${RAD_dir}/*.proper.rmdup.bam > ${pop}.no0000.bamlist
nInd=$(wc -l ${pop}.no0000.bamlist | awk '{print $1}')
mInd=$((${nInd}/2))
...
### 	and create a 0 or 1 matrix to calc cov matrix from 						 ###
	...
	angsd -bam ${pop}.no0000.bamlist -out ${pop}_pcai.noMinMAF -doMajorMinor 1 -minMapQ 20 -minQ 20 -SNP_pval 1e-12 -GL 1 -doMaf 1 -minInd ${mInd}  -doCov 1 -doIBS 1 -doCounts 1
